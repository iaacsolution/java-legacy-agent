...
5. **[Fait]** Garde-fou anti-`logRequests(true)`/`logResponses(true)` dans `hooks/pre-commit` ET `.git/hooks/pre-commit` — bloque toute réintroduction du flag LangChain4j qui loguerait aussi les headers HTTP (donc le header `x-api-key` en clair, voir Fenêtre 3 plus bas sur le pourquoi ce flag a été tenté puis rejeté). Testé par exécution réelle : staging d'un diff avec `.logRequests(true)` → hook bloque (exit 1) ; sans le flag → passe normalement.
6. Clé Anthropic **dédiée au projet**, pas une clé personnelle réutilisée ailleurs — action côté [console.anthropic.com](https://console.anthropic.com) → API Keys → nouvelle clé nommée explicitement (ex. `java-legacy-agent-demo`), à faire manuellement (pas automatisable depuis le code, aucun accès console depuis cet agent). Une fois créée : remplacer la valeur dans `.env` (jamais committé), révoquer l'ancienne clé une fois la bascule confirmée.
7. **Politique de rotation** — quand et comment :
   - **Rotation immédiate (pas de délai)** si la clé a pu fuiter : commit accidentel, log/transcript exposé, capture d'écran, canal de discussion (ex. incident déjà survenu cette session côté `jira-java-router` avec un webhook Slack — même réflexe pour une clé API).
   - **Rotation avant toute démo publique/entretien** où le poste pourrait être partagé à l'écran ou enregistré — la clé de démo n'a pas besoin de survivre à l'événement.
   - **Rotation périodique de bonne hygiène**, optionnelle sur un projet perso à ce stade (pas de cadence imposée) — pertinent surtout si la clé finit un jour partagée avec quelqu'un d'autre.
   - **Comment révoquer** : console.anthropic.com → API Keys → sélectionner la clé → Delete/Revoke. Effet immédiat, irréversible. Générer la nouvelle clé *avant* de révoquer l'ancienne pour éviter un trou de service, mettre à jour `.env`, relancer les services qui l'utilisent (ils ne relisent pas `.env` à chaud).

**Limite assumée des deux hooks (secret scan, item 2 ; anti-`logRequests`, item 5)** : `.git/hooks/pre-commit` est local et non versionné — il ne protège pas un contributeur qui clone. En collaboratif, ce contrôle doit passer en CI (`hooks/pre-commit` en template n'est qu'une copie manuelle, jamais installée automatiquement à l'installation du repo). Pour un projet solo comme celui-ci, acceptable en l'état ; pour un vrai travail d'équipe, ces deux checks devraient tourner sur chaque PR (GitHub Actions, ou le framework `pre-commit` avec `pre-commit.ci`), pas seulement sur la machine de celui qui a pensé à les copier.
